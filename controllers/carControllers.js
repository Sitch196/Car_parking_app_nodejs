...
const addCar = async (req, res) => {
    try {
      ...
      const userId = req.userId; // Extracted from the token via the protect middleware
      // Insert the car into the database
      const [result] = await pool.query('INSERT INTO cars (user_id, type, model, year) VALUES (?, ?, ?, ?)', [
        userId,
        type,
        model,
        year,
...
async function getAllCars(req, res) {
    try {
      ...
      const [rows] = await pool.query('SELECT * FROM cars WHERE user_id = ?', [userId]);
